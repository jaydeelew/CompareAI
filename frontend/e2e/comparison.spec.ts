...
test.describe('Anonymous User Comparison Flow', () => {
  ...
  test('Anonymous user can perform comparison', async ({ page }) => {
    ...
    await test.step('Submit comparison', async () => {
      ...
      const results = page.locator('[data-testid^="result-card-"], .result-card, .model-response').first();
  ...
  test('Anonymous user sees sign up prompt', async ({ page }) => {
    ...
    const signUpPrompt = page.getByText(/sign up|register|create account|free account/i);
    
    // The prompt might be in a banner, modal, or inline text
    if (await signUpPrompt.isVisible({ timeout: 2000 })) {
      await expect(signUpPrompt).toBeVisible();
      
      // Verify sign up button/link is present
  ...
  test('Anonymous user can view rate limit status', async ({ page }) => {
    ...
    const rateLimitStatus = page.locator('[data-testid="rate-limit-status"], .rate-limit-status, .usage-status');
