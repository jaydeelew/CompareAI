...
test.describe('Conversation Management', () => {
  ...
  test('Create and save conversation', async ({ page }) => {
    ...
    await test.step('Verify conversation is saved', async () => {
      ...
      const conversationHistory = page.locator('[data-testid="conversation-history"], .conversation-history, .conversations-list');
      
      if (await conversationHistory.isVisible({ timeout: 5000 })) {
        // Check if the new conversation appears in the list
        const conversationItems = conversationHistory.locator('[data-testid="conversation-item"], .conversation-item');
  ...
  test('Load previous conversation', async ({ page }) => {
    ...
    await test.step('Click on a conversation', async () => {
      ...
      if (count > 0) {
        ...
        const conversationContent = page.locator('[data-testid="conversation-content"], .conversation-messages');
