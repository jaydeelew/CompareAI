...
describe('useConversationHistory', () => {
  ...
  describe('saveConversationToLocalStorage', () => {
    ...
    it('should limit to 2 conversations for anonymous users', () => {
      ...
      act(() => {
        result.current.saveConversationToLocalStorage('input1', [createModelId('gpt-4')], conversations, false);
        result.current.saveConversationToLocalStorage('input2', [createModelId('gpt-4')], conversations, false);
        result.current.saveConversationToLocalStorage('input3', [createModelId('gpt-4')], conversations, false);
      });
  ...
  describe('loadHistoryFromAPI', () => {
    ...
    it('should set loading state during fetch', async () => {
      ...
      vi.mocked(conversationService.getConversations).mockReturnValue(promise as any);
  ...
  describe('syncHistoryAfterComparison', () => {
    it('should reload history from API after comparison', async () => {
      ...
      await act(async () => {
        await result.current.syncHistoryAfterComparison('test input', [
          createModelId('gpt-4'),
        ]);
      });
