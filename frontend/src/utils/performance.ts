 ...
 */
function getRating(value: number, threshold: number, lowerIsBetter: boolean = true): 'good' | 'needs-improvement' | 'poor' {
  if (lowerIsBetter) {
 ...
 */
function reportMetric(metric: Metric, threshold: number, callback?: PerformanceCallback, lowerIsBetter: boolean = true): void {
...
export function checkPerformanceBudgets(): {
  passed: boolean;
  metrics: Array<{ name: string; value: number; threshold: number; rating: 'good' | 'needs-improvement' | 'poor' }>;
} {
  const summary = getPerformanceSummary();
  const metrics: Array<{ name: string; value: number; threshold: number; rating: 'good' | 'needs-improvement' | 'poor' }> = [];
