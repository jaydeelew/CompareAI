...
export class RegistryValidationError extends Error {
  constructor(message: string, public readonly config?: ModelRendererConfig) {
    super(message);
...
export function validateConfig(config: ModelRendererConfig): void {
  ...
  if (!config.modelId || typeof config.modelId !== 'string') {
    throw new RegistryValidationError(
      'Configuration must have a valid modelId',
      config
    );
  }
  ...
  if (!config.displayMathDelimiters || !Array.isArray(config.displayMathDelimiters)) {
    throw new RegistryValidationError(
      'Configuration must have displayMathDelimiters array',
      config
    );
  }
  ...
  if (!config.inlineMathDelimiters || !Array.isArray(config.inlineMathDelimiters)) {
    throw new RegistryValidationError(
      'Configuration must have inlineMathDelimiters array',
      config
    );
  }
  ...
  if (!config.codeBlockPreservation.enabled) {
    throw new RegistryValidationError(
      'Code block preservation must be enabled',
      config
    );
  }
  ...
  if (!config.codeBlockPreservation.extractBeforeProcessing) {
    throw new RegistryValidationError(
      'Code blocks must be extracted before processing',
      config
    );
  }
  ...
  if (!config.codeBlockPreservation.restoreAfterProcessing) {
    throw new RegistryValidationError(
      'Code blocks must be restored after processing',
      config
    );
  }
  ...
  const allDelimiters = [
    ...config.displayMathDelimiters,
    ...config.inlineMathDelimiters,
  ];
