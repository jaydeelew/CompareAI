...
class ResponseAnalyzer:
    ...
    def generate_summary_report(self, analysis_data: Dict) -> str:
        ...
        for model_id, analysis in sorted(analyses.items()):
            ...
            report.append(f"- **Display Math Delimiters:** {', '.join(analysis['delimiters']['display']) or 'None found'}")
            report.append(f"- **Inline Math Delimiters:** {', '.join(analysis['delimiters']['inline']) or 'None found'}")
            report.append(f"- **Markdown Elements:** {', '.join(analysis['markdown_elements'].keys()) or 'None found'}")
...
def main():
    ...
    parser.add_argument(
        "responses_file",
        type=str,
        help="Path to collected responses JSON file"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
