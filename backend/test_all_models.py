...
def print_results(results: List[Dict[str, Any]]):
    ...
    for provider in sorted(by_provider.keys()):
        ...
        for result in sorted(provider_results, key=lambda x: x["response_time"]):
            ...
            print(f"  {status} {timely_mark} {result['model_name']:<40} "
                  f"{result['response_time']:>6.2f}s", end="")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tests
        future_to_model = {
            executor.submit(test_model, model): model["name"]
            for model in models
        }
        ...
        for future in as_completed(future_to_model):
            ...
            try:
                ...
                # Print progress
                status = "✅" if result["success"] else "❌"
                print(f"[{completed}/{len(models)}] {status} {model_name:<40} "
